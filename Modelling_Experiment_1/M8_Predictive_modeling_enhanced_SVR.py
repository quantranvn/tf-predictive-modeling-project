...
# Plot actual vs predicted values (using inverse transformed values)
...
plt.plot([y_test_inverse.min(), y_test_inverse.max()], [y_test_inverse.min(), y_test_inverse.max()], 'k--', lw=4)
plt.show()
...
plt.hlines(y=0, xmin=y_test_inverse.min(), xmax=y_test_inverse.max(), colors='red', linestyles='--')
plt.xlabel('Actual Closed Issues Next Month')
...
# Perform cross-validation on the scaled full dataset (X_scaled, y_scaled.ravel())
cv_scores = cross_val_score(model, X_scaled, y_scaled.ravel(), cv=5, scoring='neg_mean_squared_error')
