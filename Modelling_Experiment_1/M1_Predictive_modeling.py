...
issues_df = pd.read_csv('preprocessed_tensorflow_issues.csv', parse_dates=['closed_at'])
...
monthly_closed_issues = issues_df[issues_df['state'] == 'closed'].groupby('Month').size()

# Merge the datasets and create a lagged column for closed issues
combined_data = pd.DataFrame({
    'Commits': monthly_commits,
    'ClosedIssues': monthly_closed_issues
}).fillna(0)
...
X = combined_data[['Commits', 'ClosedIssues']]
